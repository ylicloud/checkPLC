...
| **配置 DB / 运行 DB** | 默认 **810 / 811**（减少与对方工程 DB10/DB11 冲突），须与 TIA 里通检 DB 编号一致。 |
| **语音速度** | 1×～2×。新的播报会打断旧的，不排队。 |

点 **连接**。成功后右上角为「已连接」或「已连接 (Mock)」。点 **断开** 会清掉 DQ 强制再断开。

**真机必做：** 在 TIA 的 CPU 属性 → **保护与安全 → 连接机制**，勾选 **允许来自远程对象的 PUT/GET 通信**，并重新下载。S7-1200 不勾选会报 `0x81/0x04`，Web 读不到过程映像。若勾选框是灰色，先把访问级别从「完全保护」改掉。
...
真机必须已把通检程序（含 `FC_IO_Apply`、非优化的 DB810/DB811）下载到 CPU，并在**对方已有的 OB1** 里调用该 FC，否则按钮变绿但端子没电。Mock 下变绿即表示强制逻辑成功。
...
> **不勾选 PUT/GET，真机一定连不上。**  
> TIA → CPU 属性 → **保护与安全 → 连接机制** → 勾选 **允许来自远程对象的 PUT/GET 通信** → **重新下载硬件+程序**。  
> S7-1200 必做。未勾选时 Web 报 `class=0x81, code=0x04`。勾选框若是灰色：先把 **访问级别** 从「不能访问（完全保护）」改成「完全访问」或至少「读/HMI 访问」。

### 网络

- 笔记本网口与 PLC **同一网段**（例如都是 `192.168.0.x`）。
- Windows 防火墙放行 **TCP 102**（S7 通信）。
- 连接页取消 Mock，填 CPU IP，机架 0、槽位 1，配置 DB **810**、运行 DB **811**，再点连接。

### PLC 侧（工程机上事先做好）

完整步骤见 [docs/tia-import.md](docs/tia-import.md)。摘要：

1. 打开对方已组态好的工程（建议先复制一份通检用），**不要改自动分配的硬件地址**。
2. 按 [workspace/README_IMPORT.md](workspace/README_IMPORT.md) 导入 UDT、**DB810 / DB811**、`FC_IO_Apply.scl`。
3. **不要导入 Main.xml**（仓库已删除）。在对方**已有的 OB1 / Main** 里加一次 `FC_IO_Apply` 调用。
4. DB810 / DB811 取消「优化的块访问」。
5. CPU 勾选 **允许来自远程对象的 PUT/GET 通信**（见上方），重新下载**硬件 + 程序**。
...
| `Invalid address 0x05` | DB 仍是优化访问、块编号不是 810/811、或未下载到 CPU |
| 能连上但 DI 无反应 | 配置起始字节与 TIA 不一致，或该槽未勾选「启用」 |
| DQ 按钮绿、端子没电 | 未下载 `FC_IO_Apply`、未在现有 OB1 中调用，或 DB811 写失败 |
...
| `scripts/build_workspace_xml.py` | 重新生成 `workspace/` 下 UDT/DB 的 Openness XML（不生成 Main，避免覆盖对方 OB1） |
